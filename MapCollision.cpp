	...
	for (unsigned char a = 0; a < 4; a++)
	{
		...
		if (0 <= x && 0 <= y && MAP_HEIGHT > y && MAP_WIDTH > x)
		{
			...
			else //Here we only care about the collectables.
			{
				...
				else if (Cell::teleport == i_map[x][y])
				{
					std::cout << pacman.get_position().x << pacman.get_position().y << std::endl;
					
				
					Position pos;
					pos.x = 144;
					pos.y = 48;
					pacman.set_position(pos.x, pos.y);
